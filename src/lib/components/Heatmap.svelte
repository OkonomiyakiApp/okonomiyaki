      ...
      <div class="flex flex-col">
        ...
        {#each days as day}
          ...
          <div
            ...
            style={`background-color: ${getColor(userActivityMap.get(formatDate(week, day)))}; width: 15px; height: 15px;`}
            on:click={() => selectDay(formatDate(week, day))}
            title={
              userActivityMap.get(formatDate(week, day))
                ? `${userActivityMap.get(formatDate(week, day))} hours done on ${formatDate(week, day)}`
